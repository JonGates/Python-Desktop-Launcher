...
> **交付状态：2.0.0-preview.1，已生成 Windows x64 自包含单文件 EXE。** 本地交付位于 `artifacts/portable/Launcher.exe`，可直接复制到 Python 项目中。已通过 Release 编译、78 项 Core 规格、4 项 Windows 原生规格、24 张 WPF 渲染与首次配置窗口测试，以及复制单个 EXE 后的接入、重新打开和正常退出测试。人工 DPI / IME / 完整业务工作流仍待验收，具体范围见 [测试报告](docs/TESTING.md)。生成物被 Git 忽略，源码仓库不直接存放 EXE。
...
左上角显示配置中的项目名称与说明，左下角显示项目版本。标签和细窄圆角滚动条适配深浅主题；启动动作中的「添加参数」位于参数标题右侧。
