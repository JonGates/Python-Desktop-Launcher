...
设置页「启动动作」采用左侧动作列表、右侧配置的分栏布局。点击列表切换动作，新增与删除位于列表底部；右侧配置独立滚动。切换保留待保存修改，字段格式有误时先修正再切换。
...
界面语言不属于 launcher.yaml。首次启动按系统 UI 语言选择中文或英文；用户选择保存在全局的 `%LOCALAPPDATA%/ProjectLauncher/preferences.json`（仅 language 字段）。损坏偏好不阻止启动，重新保存前保留损坏副本；写入失败时本次语言选择仍有效。项目名称、动作名称、参数、业务日志和终端内容不会被翻译，也不会因切换语言写回配置。
