...
public sealed class ActionWorkspace : UserControl
{
    ...
    public event Action<Exception?>? ValidationNotice;
    private readonly Dictionary<Control, Exception> _errors = new();
    ...
    public ActionWorkspace(LauncherConfig draft, string root, Action changed)
    {
        ...
        var listTitle = LabelKey("Ui.008", true); listTitle.Margin = new(0, 0, 0, 10); listPanel.Children.Add(listTitle);
        ...
        var add = ButtonKey("Text.027", AddAction); add.Margin = new(0, 0, 0, 6); buttons.Children.Add(add);
        var delete = ButtonKey("Text.028", DeleteAction); delete.Margin = new(0); buttons.Children.Add(delete);
    ...
    private static TextBlock LabelKey(string key, bool title = false) => Label("", title).Localize(TextBlock.TextProperty, key);
    private Button ButtonKey(string key, Action click) => Button("", click).Localize(ContentControl.ContentProperty, key);
    ...
    { var b = new Button { Content = text, Padding = new(10, 6, 10, 6), MinHeight = 32, Margin = new(4, 0, 0, 0) }; b.Click += (_, _) => { if (!CanRebuild()) return; try { click(); } catch (Exception ex) { ValidationNotice?.Invoke(ex); } }; return b; }
    ...
    private void Edit(Control field, Action write)
    {
        ...
        catch (Exception ex) { _errors[field] = ex; field.Dynamic(ToolTipProperty, () => ex.Message); }
    ...
    public void Commit()
    {
        if (_errors.Count != 0) throw _errors.First().Value;
        foreach (var action in _draft.Actions)
            if (ActionEditing.CommandKind(action) is "script" or "module" && string.IsNullOrWhiteSpace(action.Argv.Last())) throw new ConfigException(new ProjectLauncher.Core.Localization.LocalizedDiagnostic("Text.029", []));
    ...
    {
        if (_errors.Count != 0) { ValidationNotice?.Invoke(_errors.First().Value); return; }
        RefreshActions(ActionEditing.AddAction(_draft)); _changed();
    }
    private void DeleteAction()
    {
        if (_action is null || !Dialogs.Confirm(Window.GetWindow(this), L.Text("Text.031"), _action.ToString(), L.Text("Text.032"), true)) return;
    ...
    private bool Membership()
    {
        ...
        if (!Dialogs.Confirm(Window.GetWindow(this), L.Text("Text.033"), L.Text("Text.034"), L.Text("Text.035"))) return false;
    ...
    private void Rebuild()
    {
        ...
        if (_action is null) _editor.Children.Add(LabelKey("Text.036"));
        else
        {
            var action = _action;
            ...
            var kinds = new[] { "script", "module", "program", "advanced" };
            ...
            var kindBox = Choice(kinds, kinds[index], v => {
                ...
                if (!Dialogs.Confirm(Window.GetWindow(this), L.Text("Text.041"), L.Text("Text.042"), L.Text("Text.043"))) { Dispatcher.InvokeAsync(Rebuild); return; }
                action.Argv = v == kinds[0] ? ["python", ""] : v == kinds[1] ? ["python", "-m", ""] : [""];
                if (v == kinds[3]) action.Argv = ["python", "-c", ""];
                Dispatcher.InvokeAsync(Rebuild);
            });
            L.Options(kindBox, new Dictionary<string, string> { ["script"] = "Text.037", ["module"] = "Text.038", ["program"] = "Text.039", ["advanced"] = "Text.040" });
            _editor.Children.Add(Pair("Text.044", nameBox, "Text.045", kindBox));
            if (kind == "advanced")
            {
                _editor.Children.Add(LabelKey("Text.046"));
                _editor.Children.Add(Text(string.Join("\n", action.Argv), v => action.Argv = v.Replace("\r\n", "\n").Split('\n').ToList(), true));
            }
            else
            {
                _editor.Children.Add(LabelKey(kind == "module" ? "Text.047" : "Text.048"));
                int target = kind == "script" ? 1 : kind == "module" ? 2 : 0;
                var box = Text(action.Argv[target], v => action.Argv[target] = v);
                var line = new DockPanel();
                if (kind != "module") { var browse = ButtonKey("Text.049", () => { var root = new ProjectEnvironment(_draft, Path.Combine(_root, "launcher.yaml")).Root; var d = new OpenFileDialog { InitialDirectory = root, Filter = kind == "script" ? L.Text("Text.050") : L.Text("Text.051") }; if (d.ShowDialog(Window.GetWindow(this)) == true) box.Text = ActionEditing.BrowsedTarget(root, d.FileName); }); DockPanel.SetDock(browse, Dock.Right); line.Children.Add(browse); }
                line.Children.Add(box); _editor.Children.Add(line);
            }
            var parameterHeading = new DockPanel { Margin = new(0, 8, 0, 10) };
            var addParameter = ButtonKey("Text.052", () => EditParameter(null, action));
            DockPanel.SetDock(addParameter, Dock.Right); parameterHeading.Children.Add(addParameter);
            var heading = LabelKey("Ui.054", true); heading.Margin = new(0); heading.VerticalAlignment = VerticalAlignment.Center;
            ...
            var advanced = new StackPanel(); advanced.Children.Add(LabelKey("Text.054")); advanced.Children.Add(new TextBox { Text = action.Id, IsReadOnly = true });
            advanced.Children.Add(LabelKey("Text.055")); advanced.Children.Add(Text(action.TimeoutSeconds.ToString(), v => action.TimeoutSeconds = int.TryParse(v, out int n) && n >= 0 ? n : throw new ConfigException(new ProjectLauncher.Core.Localization.LocalizedDiagnostic("Text.056", []))));
            _editor.Children.Add(new Expander { Header = "", Content = advanced, Margin = new(0, 12, 0, 0) }.Localize(HeaderedContentControl.HeaderProperty, "Text.057"));
        }
        _building = false; RefreshPreview();
    }
    private void EditParameter(ParameterDefinition? parameter, ActionDefinition action)
    {
        var dialog = new ParameterEditorWindow(parameter ?? new() { Label = L.Text("Text.058"), Argument = "--parameter" }, parameter is null, edited => {
            bool needsConfirmation = _draft.Actions.Any(a => a.Parameters is null);
            if (needsConfirmation && !Dialogs.Confirm(Window.GetWindow(this), L.Text("Text.033"), L.Text("Text.059"), L.Text("Text.060")))
                throw new ConfigException(new ProjectLauncher.Core.Localization.LocalizedDiagnostic("Text.061", []));
    ...
    private void AddParameterRow(ParameterDefinition p, ActionDefinition action)
    {
        ...
        buttons.Children.Add(ButtonKey("Text.062", () => EditParameter(p, action)));
        buttons.Children.Add(Button("↑", () => Move(p, -1))); buttons.Children.Add(Button("↓", () => Move(p, 1)));
        buttons.Children.Add(ButtonKey("Text.063", () => { if (!Membership()) return; action.Parameters!.Remove(p.Name); _changed(); Rebuild(); }));
        ...
        var detail = Label("").Dynamic(TextBlock.TextProperty, () => {
            string binding = p.Binding == "env" ? "ENV " + p.EnvName : p.Binding == "positional" ? L.Text("Text.064") : p.Argument;
            string value = p.IsSecret ? L.Text("Text.065") : p.Default is null ? L.Text("Text.066") : L.Text("Text.067") + ValueCodec.Text(p.Default);
            return binding + "  ·  " + p.Type + "  ·  " + value + (p.Required ? L.Text("Text.068") : "");
        });
    ...
    private static Grid Pair(string firstLabel, UIElement first, string secondLabel, UIElement second)
    {
        ...
        var left = new StackPanel(); left.Children.Add(LabelKey(firstLabel)); left.Children.Add(first); row.Children.Add(left);
        var right = new StackPanel(); right.Children.Add(LabelKey(secondLabel)); right.Children.Add(second); Grid.SetColumn(right, 2); row.Children.Add(right);
        return row;
    }
